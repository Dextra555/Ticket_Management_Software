...
class Subject_model extends CI_Model {
	...
	public function save_subject_details($data)
	{
	...
	public function clicked_subject_delete_action($get_id)
	{
		  $this->db->where('id', $get_id);
 		  $this->db->delete('subject'); 
 		  return 'Deleted success';
	}
	public function clicked_device_password_updations($id_value_stored,$change_password_subject)
	{
		$sql = "UPDATE subject SET password='".sha1($change_password_subject)."' WHERE id='$id_value_stored'";
		$this->db->query($sql);
	...
	{
		$this->db->insert('subject_new_role', $data);
		$this->db->select('*');
		$this->db->from('subject_new_role');
		$this->db->order_by('id','DESC');
		$this->db->limit(1);			
		return $this->db->get()->result_array();
	}
	public function get_subject_role_details()
	{
		$this->db->select('*');
		$this->db->from('subject_new_role');
		return $this->db->get()->result_array();
	...
	public function clicked_view_edit_subject($get_id)
	{
		 $this->db->select('*');
		 $this->db->from('subject');
		 $this->db->where('id', $get_id);
	...
	{
		  $this->db->where('id', $get_id);
 		  $this->db->delete('subject_new_role'); 
 		  return 'Deleted success';
	}
	...
	{
		$sql = "UPDATE subject_new_role SET role_name='$role_name', role_desc='$role_desc' WHERE id='$get_id'";
		$this->db->query($sql);
		$this->db->select('*');
		$this->db->from('subject_new_role');
		$this->db->where('id',$get_id);
    ...
    {
    	$sql = "SELECT COUNT(subject_status) as subject_count FROM subject WHERE subject_status = '1'";
    	...
    	$count2 = array_merge($subject_count,$staff_count);
